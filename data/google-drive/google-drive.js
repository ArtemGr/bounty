...
async function pocCreate() {
  ...
  const timeline = fs.createReadStream ('c:/spool/synced/iPhone/timeline.mp4')

  if (fs.existsSync ('full-token.json')) {
    const tokenˢ = await fsp.readFile ('full-token.json', {encoding: 'utf8'})
    /** @type {Credentials} */
    ...
    await fsp.writeFile ('full-token.json', JSON.stringify (token.tokens))
...
async function pocUpdate() {
  ...
  // ⌥ load the limited “drive.file” token
  // ⌥ save the limited “drive.file” token
}
